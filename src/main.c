...
// Temperatura inicial Tmax
  ...
  if(opcion=='l'){
    double Tmax=2.0, Tmin = 0.1, T = Tmax;
    int n=33;
    int N_prom=10;  // Cantidad de muestras por temperatura
    int Term = 2000;
    double rho=0.8;
    if(argc>2) sscanf(argv[2],"%lg",&rho);
    if(argc>3) sscanf(argv[3],"%d",&n);
    if(argc>4) sscanf(argv[4], "%d", &N_prom);
    if(argc>5) sscanf(argv[5], "%d", &Term);
    //if(argc>6) sscanf(argv[6], "%d", &Term);
    int secs = time(NULL);
    int N = 125;
    ...
    E[0] = Energia_Cinetica(vector,N,m)+Energia_Potencial(vector,N,LUTP,Ntable,L);
    D[0] = Lindemann(vector, &vector_fuerza, N,LUTF, Ntable, m, h, L, N_prom,Tr);
    ...
    for(int j=1;j<n;j++){
      ...
      E[j] = Energia_Cinetica(vector,N,m)+Energia_Potencial(vector,N,LUTP,Ntable,L);
      D[j] = Lindemann(vector, &vector_fuerza, N,LUTF, Ntable, m, h, L,N_prom,Tr+j);
    }
    char name[100];
    sprintf(name, "Lindemann_%d_%1.3f.txt", N,rho);
    FILE* fp = fopen(name, "w");
    for(int j=0;j<n;j++) fprintf(fp,"%f %f %f\n", E[j], Tr[j], D[j]);
    fclose(fp);
