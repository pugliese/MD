...
/*
    ...
    v[0] = Orden_verlet(vector, N, a);
    for(int i=1;i<N_pasos;i++){
      Verlet(vector,&vector_fuerza,N,LUTF, Ntable,m,h,L);
      v[i] = Orden_verlet(vector, N, a);
    }
    FILE* fp = fopen("Verlet.txt", "w");
    ...
    Inicializar(vector,vector_fuerza, N,LUTF,Ntable, rho, m,T);
    double* P = malloc(N_pasos*sizeof(double));
    for(int i=0;i<N_pasos;i++){
      Pex=Verlet(vector,&vector_fuerza,N,LUTF, Ntable,m,h,L);
      Ecin = Energia_Cinetica(vector, N, m);
      P[i] = Presion(Pex, Ecin, N, rho);
    }
    FILE* fp = fopen("Presion.txt", "w");
    for(int i=0;i<N_pasos;i++){
      fprintf(fp, "%lg ", P[i]);
    }
    printf("\n");
